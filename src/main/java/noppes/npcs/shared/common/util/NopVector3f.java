...
public class NopVector3f implements Comparable<NopVector3f> {
    ...
    public NopVector3f normalize() {
        float f0 = x * x + y * y + z * z;
        if (f0 < 1.17549435E-38F) { return this; }
        float f1 = 0.5f * f0;
        long l = Double.doubleToRawLongBits(f0);
        l = 6910469410427058090L - (l >> 1);
        f0 = (float) Double.longBitsToDouble(l);
        return mul(f0 * (1.5f - f1 * f0 * f0));
    }
    ...
    public boolean equals(Object ob) {
        ...
        if (ob instanceof NopVector3f) {
            return x == ((NopVector3f) ob).x && y == ((NopVector3f) ob).y && z == ((NopVector3f) ob).z;
        }
        return false;
    }
