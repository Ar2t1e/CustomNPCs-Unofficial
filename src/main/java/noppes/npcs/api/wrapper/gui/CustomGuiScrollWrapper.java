...
public class CustomGuiScrollWrapper extends CustomGuiComponentWrapper implements IScroll {
   ...
   public CompoundTag toNBT(CompoundTag compound) {
      ...
      compound.putIntArray("selection", selection);
      compound.put("list", Arrays.stream(list).map(StringTag::valueOf).collect(Collectors.toCollection(ListTag::new)));
   ...
   public CustomGuiComponentWrapper fromNBT(CompoundTag compound) {
      ...
      setList(compound.getList("list", 8).stream().map(Tag::getAsString).toArray(String[]::new));
