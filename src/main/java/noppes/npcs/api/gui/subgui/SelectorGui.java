...
public class SelectorGui {
   ...
   public static CustomGuiWrapper openFaction(int id, IPlayer<?> player, SelectorGui.SelectionCallback callback) {
      ...
      IScroll scroll = gui.addScroll(1, 4, 14, 272, 200, FactionController.instance.list().stream()
              .map(IFaction::getName)
              .toArray(String[]::new))
   ...
   public interface SelectionCallback {
      void call(@ParamName("selected") int selected);
   }
   ...
   static class DialogSelectorGui {
      ...
      public DialogSelectorGui(CustomGuiWrapper gui, int id, SelectorGui.SelectionCallback callback) {
         ...
         gui.getBackgroundRect().setTextureOffset(0, 0);
         gui.getBackgroundRect().setRepeatingTexture(64, 64, 4);
         IButton b = gui.addTexturedButton(666, "X", 352, -4, 14, 14, CustomNpcs.MODID + ":textures/gui/components.png", 0, 64);
         b.getTextureRect().setRepeatingTexture(64, 22, 3);
         b.setTextureHoverOffset(22).setHoverText("gui.close");
         b.setOnPress((guii, bb) -> guii.close());
         gui.addLabel(0, "gui.categories", 8, 4, 10, 8);
         gui.addLabel(1, "dialog.dialogs", 175, 4, 10, 8);
         IScroll scrollCategories = gui.addScroll(2, 4, 14, 170, 208, DialogController.instance.categories().stream().map(IDialogCategory::getName).toArray(String[]::new)).setOnClick((gui2, scroll2) -> {
            this.category = categoryData.get(scroll2.getSelectionList()[0]);
            this.fill();
            gui2.update();
         });
         if (this.dialog != null) {
         ...
         this.dialogScroll = gui.addScroll(3, 175, 14, 170, 208).setOnClick((gui2, scroll2) -> {
            this.dialog = this.data.get(scroll2.getSelectionList()[0]);
      ...
      private void fill() {
         ...
         this.data = data;
         this.dialogScroll.setList(data.values().stream().map(IDialog::getName).toArray(String[]::new));
   ...
   static class QuestSelectorGui {
      ...
      public QuestSelectorGui(CustomGuiWrapper gui, int id, SelectorGui.SelectionCallback callback) {
         ...
         IScroll scrollCategories = gui.addScroll(2, 4, 14, 170, 208, QuestController.instance.categories().stream()
                 .map(IQuestCategory::getName)
                 .toArray(String[]::new))
         ...
         this.questScroll = gui.addScroll(3, 175, 14, 170, 208).setOnClick((gui2, scroll2) -> {
            this.quest = this.data.get(scroll2.getSelectionList()[0]);
      ...
      private void fill() {
         ...
         this.data = data;
         this.questScroll.setList(data.values().stream().map(Quest::getName).toArray(String[]::new));
