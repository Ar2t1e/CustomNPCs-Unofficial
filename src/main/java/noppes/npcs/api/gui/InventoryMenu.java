...
public class InventoryMenu extends MainMenuGui {
   ...
   public InventoryMenu(EntityCustomNpc npc, IPlayer<?> player) {
      ...
      for(int i = 0; i < 20; ++i) {
         float chance = 100.0F;
         //if (npc.inventory.dropChances.containsKey(i)) { chance = npc.inventory.dropChances.get(i); }
         ...
         int finalI = i;
         //panel.addItemSlot(0, i * 22 + 1, npc.inventory.getDropItem(i)).setOnUpdate((gui, slot) -> npc.inventory.setDropItem(finalI, slot.getStack(), ((ISlider)panel.getComponent(finalI + 20)).getValue()));
         //panel.addSlider(20 + i, 20, i * 22, 150, 18, "%s %%").setDecimals(2).setValue(chance).setOnChange((gui, slider) -> npc.inventory.setDropItem(finalI, npc.inventory.getDropItem(finalI), slider.getValue()));
      }
