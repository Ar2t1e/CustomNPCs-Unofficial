...
public class HealthMenu extends MainMenuGui {
   public HealthMenu(EntityCustomNpc npc, IPlayer<?> player) {
      ...
      CustomGuiTextFieldWrapper var10000 = panel.addTextField(1, 0, y + 9, 80, 20).setCharacterType(1);
      var10000.setText(stats.maxHealth + "").setOnChange((gui, textfield) -> {
         stats.setMaxHealth(textfield.getInteger());
         npc.heal((float)stats.maxHealth);
      });
      panel.addLabel(4, "stats.regenhealth", 100, y, 100, 8);
      var10000 = panel.addTextField(5, 100, y + 9, 80, 20).setCharacterType(1);
      var10000.setText(stats.healthRegen + "").setOnChange((gui, textfield) -> stats.setHealthRegen(textfield.getInteger()));
      panel.addLabel(6, "stats.combatregen", 200, y, 100, 8);
      var10000 = panel.addTextField(7, 200, y + 9, 80, 20).setCharacterType(1);
      var10000.setText(stats.combatRegen + "").setOnChange((gui, textfield) -> stats.setCombatRegen(textfield.getInteger()));
      y += 34;
      ...
      panel.addSlider(22, 0, y + 9, 150, 18, "%s %%").setMin(-100.0F).setValue((npc.stats.resistances.get("knockback") - 1.0F) * 100.0F).setOnChange((gui, slider) -> npc.stats.resistances.data.put("knockback", slider.getValue() / 100.0F + 1.0F));
      panel.addLabel(23, "stats.explosion", 160, y, 100, 8);
      panel.addSlider(24, 160, y + 9, 150, 18, "%s %%").setMin(-100.0F).setValue((npc.stats.resistances.get("explosion") - 1.0F) * 100.0F).setOnChange((gui, slider) -> npc.stats.resistances.data.put("explosion", slider.getValue() / 100.0F + 1.0F));
      y += 34;
      panel.addLabel(25, "stats.melee", 0, y, 100, 8);
      panel.addSlider(26, 0, y + 9, 150, 18, "%s %%").setMin(-100.0F).setValue((npc.stats.resistances.get("melee") - 1.0F) * 100.0F).setOnChange((gui, slider) -> npc.stats.resistances.data.put("melee", slider.getValue() / 100.0F + 1.0F));
      panel.addLabel(27, "item.minecraft.arrow", 160, y, 100, 8);
      panel.addSlider(28, 160, y + 9, 150, 18, "%s %%").setMin(-100.0F).setValue((npc.stats.resistances.get("arrow") - 1.0F) * 100.0F).setOnChange((gui, slider) -> npc.stats.resistances.data.put("arrow", slider.getValue() / 100.0F + 1.0F));
   }
