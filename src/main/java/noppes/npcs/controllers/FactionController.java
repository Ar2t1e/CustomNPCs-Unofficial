...
public class FactionController implements IFactionHandler {
			...
			if (saveDir != null) {
				try {
					...
					try {
						...
						if (file2.exists()) { loadFactionsFile(file2); }
	...
	private void loadFactionsFile(File file) throws IOException {
		DataInputStream inputStream = new DataInputStream(new BufferedInputStream(new GZIPInputStream(Files.newInputStream(file.toPath()))));
	...
	public void saveFaction(Faction faction) {
		if (faction.id < 0) {
			...
			while (hasName(faction.name)) { faction.name += "_"; }
		} else {
		...
		factions.put(faction.id, faction);
		Packets.sendAll(new PacketSyncUpdate(faction.id, 1, faction.save(new NBTTagCompound())));
		saveFactions();
	}
	...
	@Override
	public IFaction get(int id) { return factions.get(id); }
