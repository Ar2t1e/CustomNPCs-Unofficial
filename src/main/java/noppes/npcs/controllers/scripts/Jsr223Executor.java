...
public class Jsr223Executor
        ...
        if (!errored && engine != null && !unknownFunctions.contains(functionName)) {
            ...
            try (PrintWriter pw = new PrintWriter(sw)){
                ...
                try {
                    ...
                    errored = true;
                    Component notice = handler.noticeString(functionName, event);
                    ...
                    if (errorText.contains("" + (char) 10)) {
                        for (int c = 0; c < errorText.length(); c++) {
                            ...
                            if (errorText.charAt(c) == 10) { error.append(TextFormatting.DARK_GRAY); }
                        }
                    }
                    else { error = new StringBuilder(TextFormatting.DARK_GRAY + "" + t); }
    ...
    public void setErrored(boolean erroredIn) { errored = erroredIn; }
