...
public class ScriptContainer {
   ...
   public CompoundTag save(CompoundTag compound) {
      ...
      compound.putBoolean("isClient", isClient);
         ...
         if (engine != null) {
            ...
            synchronized(lock) {
               ...
               try {
                  ...
                  errored = true;
                  MutableComponent notice = handler.noticeString(type, event);
   ...
   public void setEngine(String scriptLanguage) {
      if (currentScriptLanguage == null || !currentScriptLanguage.equals(scriptLanguage)) {
         ...
         canEncryptCode = false;
         if (!errored) {
            for (Entry<String, Object> entry : Data.entrySet()) { engine.put(entry.getKey(), entry.getValue()); }
            fillEngine(engine);
   ...
   public void fillEngine(ScriptEngine scriptEngine) {
      scriptEngine.put("dump", new Dump());
      scriptEngine.put("log", new Log());
