...
public class AnimationContainer {
   ...
   public AnimationContainer(int animation, String part, int length, float speed, boolean additional, boolean loop) {
      ...
      this.scale = new NopVector3f[this.actualLength];
   }
   ...
   public void step(ModelPartWrapper part, int step, float progress) {
      ...
      if (this.startupTicks < 60) {
         ...
         if (this.hasScale) {
            part.setScale(part.getScale().lerp(this.scale[i].lerp(this.scale[j], progress), 0.15F));
         } else {
         ...
         if (this.hasScale) {
            part.setScale(this.scale[i].lerp(this.scale[j], progress));
         }
      }
   ...
   public AnimationContainer copy() {
      ...
      container.hasScale = this.hasScale;
      ...
      for(int i = 0; i < this.actualLength; ++i) {
         if (i < this.length) {
            ...
            if (this.hasScale) {
               container.scale[i] = this.scale[i];
            }
         } else {
            if (this.hasTranslate) {
            ...
            if (this.hasScale) {
               container.scale[i] = container.scale[this.length - i % this.length - 2];
            }
         }
      }
      return container;
