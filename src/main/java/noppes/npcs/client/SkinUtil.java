...
public class SkinUtil {
   ...
   public static void resetSkin(UUID uuid) {
      ...
      Map<MinecraftProfileTexture.Type, ResourceLocation> map = ((IPlayerInfoMixin) playerInfo).getTextureLocations();
      map.clear();
   ...
   public static void createPlayerSkin(SkinData skinData) {
      ...
      if (skinData.isUrl()) {
         ...
         String locSkin = String.format("assets/%s/%s", location.getNamespace(), location.getPath());
      ...
      if (createdSkins.contains(location)) { return; }
      String locSkin = String.format("assets/%s/%s", skinData.getLocation().getNamespace(), skinData.getLocation().getPath());
      File file = new File(CustomNpcs.Dir, locSkin);
   ...
   private static BufferedImage readBufferedImage(ResourceManager resourceManager, ResourceLocation location) {
      if (resourceManager == null || location == null) { return null; }
      InputStream imageStream = null;
      try {
         Optional<Resource> p = resourceManager.getResource(location);
         if (p.isPresent()) {
            imageStream = p.get().open();
            return ImageIO.read(imageStream);
         }
      }
      catch (IOException ignored) { }
      finally { IOUtils.closeQuietly(imageStream); }
      return null;
   }
