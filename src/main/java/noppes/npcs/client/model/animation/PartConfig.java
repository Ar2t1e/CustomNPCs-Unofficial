...
public class PartConfig implements IAnimationPart {
    ...
    public void load(CompoundTag compound) {
        int v = compound.getInt("v");
        float pi = (float) Math.PI;
        for (int i = 0; i < 5; i++) {
            float valueR = compound.getList("Rotation", 5).getFloat(i);
            float valueO = compound.getList("Offset", 5).getFloat(i);
            float valueS = compound.getList("Scale", 5).getFloat(i);
            try {
        ...
        if (compound.contains("Disabled", 1)) { disable = compound.getBoolean("Disabled"); }
        if (compound.contains("Show", 1)) { show = compound.getBoolean("Show"); }
        if (compound.contains("Name", 8)) { name = compound.getString("Name"); }
        setMainName();
        if (compound.contains("Type", 3)) {
            int t = compound.getInt("Type");
            if (t < 0) { t *= -1; }
            type = EnumParts.values()[t % EnumParts.values().length];
        }
    }
    ...
    public CompoundTag save() {
        ...
        ListTag listOff = new ListTag();
        ListTag listSc = new ListTag();
        for (int i = 0; i < 3; i++) {
            listRot.add(FloatTag.valueOf(this.rotation[i]));
            listOff.add(FloatTag.valueOf(this.offset[i]));
            listSc.add(FloatTag.valueOf(this.scale[i]));
        }
        listRot.add(FloatTag.valueOf(this.rotation[3]));
        listRot.add(FloatTag.valueOf(this.rotation[4]));
        compound.put("Rotation", listRot);
        compound.put("Offset", listOff);
        compound.put("Scale", listSc);

        compound.putInt("Part", this.id);
        compound.putBoolean("Disabled", this.disable);
        compound.putBoolean("Show", this.show);
