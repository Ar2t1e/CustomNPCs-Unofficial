...
public class AnimationFrameConfig implements IAnimationFrame {
    ...
    public void load(CompoundTag compound) {
        ...
        if (compound.contains("StartSound", 8)) { this.setStartSound(compound.getString("StartSound")); }
        if (compound.contains("EmotionID", 3)) { this.setStartEmotion(compound.getInt("EmotionID")); }
        if (compound.contains("IsNowDamage", 1)) { this.isNowDamage = compound.getBoolean("IsNowDamage"); }
        if (compound.contains("ShowStacks", 7)) {
        ...
        this.setHoldRightStackType(compound.getInt("HoldRightType"));
        this.setHoldLeftStackType(compound.getInt("HoldLeftType"));
        NpcAPI api = NpcAPI.Instance();
        if (compound.contains("HoldRightStack", 10)) {
            assert api != null;
            this.setHoldRightStack(api.getIItemStack(ItemStack.of(compound.getCompound("HoldRightStack")))); }
        if (compound.contains("HoldLeftStack", 10)) {
            assert api != null;
            this.setHoldLeftStack(api.getIItemStack(ItemStack.of(compound.getCompound("HoldLeftStack")))); }

        this.parts.clear();
        for (int i = 0; i < compound.getList("PartConfigs", 10).size(); i++) {
            CompoundTag nbt = compound.getList("PartConfigs", 10).getCompound(i);
            PartConfig pc;
            if (nbt.contains("Part", 3) && this.parts.containsKey(nbt.getInt("Part"))) { pc = this.parts.get(nbt.getInt("Part")); }
        ...
        if (compound.contains("Motion", 9) && ((ListTag) Objects.requireNonNull(compound.get("Motion"))).getElementType() == 5) {
            for (int i = 0; i < 3 && i < compound.getList("Motion", 5).size(); i++) {
                motions[i] = compound.getList("Motion", 5).getFloat(i);
            }
        }
        damageHitboxes.clear();
        ListTag list = compound.getList("DamageHitboxes", 10);
        for (int i = 0; i < list.size(); i++) {
            damageHitboxes.put(i, new AnimationDamageHitbox(list.getCompound(i), i));
        }
    ...
    public static EnumParts getPartType(int id) {
        return switch (id) {
            case 0 -> EnumParts.HEAD;
            case 1 -> EnumParts.ARM_LEFT;
            case 2 -> EnumParts.ARM_RIGHT;
            case 3 -> EnumParts.BODY;
            case 4 -> EnumParts.LEG_LEFT;
            case 5 -> EnumParts.LEG_RIGHT;
            case 6 -> EnumParts.LEFT_STACK;
            case 7 -> EnumParts.RIGHT_STACK;
            default -> EnumParts.CUSTOM;
        };
    }
    ...
    public CompoundTag writeNBT() {
        ...
        compound.putBoolean("IsSmooth", this.smooth);
        compound.putBoolean("IsNowDamage", this.isNowDamage);
        compound.putInt("ID", this.id);
        compound.putInt("Speed", this.speed);
        compound.putInt("EndDelay", this.delay);
        ListTag list = new ListTag();
        for (int id : this.parts.keySet()) {
            list.add(this.parts.get(id).save());
        }
        compound.put("PartConfigs", list);
        ListTag listM = new ListTag();
        for (int i = 0; i < 3; i++) {
            listM.add(FloatTag.valueOf(motions[i]));
        }
        compound.put("Motion", listM);
        compound.putString("StartSound", this.getStartSound());
        compound.putInt("EmotionID", this.emotionId);
        compound.putInt("Version", 1);
        compound.putInt("HoldRightType", this.holdRightType);
        compound.putInt("HoldLeftType", this.holdLeftType);
        compound.put("HoldRightStack", this.holdRightStack.getMCItemStack().save(new CompoundTag()));
        compound.put("HoldLeftStack", this.holdLeftStack.getMCItemStack().save(new CompoundTag()));
        compound.putByteArray("ShowStacks", new byte[] { (byte) (showMainHand ? 1 : 0), (byte) (showOffHand ? 1 : 0), (byte) (showHelmet ? 1 : 0), (byte) (showBody ? 1 : 0), (byte) (showLegs ? 1 : 0), (byte) (showFeets ? 1 : 0) });

        ListTag listDHB = new ListTag();
        int i = 0;
        for (AnimationDamageHitbox aDHB : damageHitboxes.values()) {
            aDHB.id = i;
            listDHB.add(aDHB.getNBT());
            i++;
        }
        compound.put("DamageHitboxes", listDHB);
    ...
    public void setStartSound(String sound) {
        ...
        if (sound != null) {
            ...
            if (this.sound.getPath().isEmpty() || this.sound.getPath().isEmpty()) {
                this.sound = null;
            }
        }
    }
    ...
    public void setRotationAngles(ModelNpcAlt<EntityCustomNpc> model) {
