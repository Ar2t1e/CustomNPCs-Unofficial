...
public class TranslateUtil {
   ...
   private static String translateGoogle(String textLanguageKey, String translationLanguageKey, String originalText) {
      ...
      try {
         URLConnection connection = new URL(String.format(TranslateUrl, textLanguageKey, translationLanguageKey,
                 URLEncoder.encode(originalText, StandardCharsets.UTF_8))).openConnection();
   ...
   public static void speak(IPlayer<?> player, String languageKey, String text, float volume) {
      ...
      Packets.send((ServerPlayer) player.getMCEntity(), new PacketSpeak(languageKey, text, volume));
   }

   /**
    * Text-to-speech via Google Translate TTS.
    * @param level world in which to reproduce translation
    ...
    */
   @SuppressWarnings("all")
   public static void speak(IWorld level, IPos pos, int range, String languageKey, String text, float volume) {
      if (level == null || level.getMCLevel() == null || pos == null) { return; }
      if (level.getMCLevel().isClientSide()) {
         MusicController.Instance.speak(languageKey, text, volume);
         return;
      }
      Packets.sendNearby(level.getMCLevel(), pos.getMCBlockPos(), Math.max(range, 3), new PacketSpeak(languageKey, text, volume));
   }
