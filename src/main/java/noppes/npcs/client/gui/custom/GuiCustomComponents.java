...
public class GuiCustomComponents {
   ...
   public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
      for (IComponentGui component : components.values()) {
         if (component instanceof Renderable renderable) { renderable.render(graphics, mouseX, mouseY, partialTicks); }
      }
      for (IItemSlot slot : slots) {
         if (slot.getGuiType() > 0) { renderSlot(graphics, slot); }
      }
      for (IComponentGui component : components.values()) {
         if (component instanceof PostRenderable renderable) { renderable.postRender(graphics, mouseX, mouseY, partialTicks); }
      }
   }
   ...
   public void containerTick() {
      for (IComponentGui component : components.values()) {
         if (component instanceof EditBox editBox) { editBox.tick(); }
         if (component instanceof CustomGuiSlider slider) { slider.tick(); }
      }
   }

   public boolean charTyped(char typedChar, int keyCode) {
      for (IComponentGui comp : components.values()) {
         if (comp instanceof GuiEventListener guiEvent && guiEvent.charTyped(typedChar, keyCode)) { return true; }
      }
      return false;
   }
   ...
   public boolean keyPressed(int key, int key_1, int key_2) {
      for (IComponentGui comp : components.values()) {
         if (comp instanceof GuiEventListener guiEvent && guiEvent.keyPressed(key, key_1, key_2)) { return true; }
      }
      return false;
   }
   ...
   public boolean mouseDragged(double x, double y, int button, double dx, double dy) {
      for (IComponentGui comp : components.values()) {
         if (comp instanceof GuiEventListener guiEvent && comp.getId() == draggingId && guiEvent.mouseDragged(x, y, button, dx, dy)) {
            return true;
         }
      }
      return false;
   ...
   public boolean mouseReleased(double x, double y, int button) {
      for (IComponentGui comp : components.values()) {
         if (comp.getId() == draggingId && comp instanceof GuiEventListener guiEvent && guiEvent.mouseReleased(x, y, button)) {
            draggingId = -1;
            return true;
         }
      }
