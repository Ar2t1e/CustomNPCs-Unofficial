...
public class SubGuiEditText extends GuiBasic implements ITextfieldListener {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (imageWidth > 256) {
			drawTexturedModalRect(0, imageHeight - 1, 0, 218, 250, imageHeight);
			drawTexturedModalRect(250, imageHeight - 1, 256 - (imageWidth - 250), 218, imageWidth - 250, imageHeight);
		}
		else { drawTexturedModalRect(0, imageWidth - 1, 0, 218, imageWidth, 4); }
	...
	public void unFocused(GuiTextFieldNop textField) {
		if (!textField.getValue().isEmpty() &&
				...
				hovers.get(0).get(0).getContents() instanceof TextComponentTranslation &&
				((TextComponentTranslation) hovers.get(0).get(0).getContents()).getKey().equals("hover.player")) {
