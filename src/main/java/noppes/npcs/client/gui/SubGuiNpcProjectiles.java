...
public class SubGuiNpcProjectiles extends GuiBasic implements ITextfieldListener {
   ...
   static {
      ...
      for(PotionEffectType ept : PotionEffectType.values()) {
         MobEffect pt = PotionEffectType.getMCType(ept.get());
         if (pt != null) { list.add(pt.getDescriptionId()); }
      }
   ...
   public void buttonEvent(GuiButtonNop button) {
      switch (button.id) {
         ...
         case 4: {
            int effect = button.getValue() == potionNames.length - 1 ? 666 : button.getValue();
