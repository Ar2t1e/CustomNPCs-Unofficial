...
public class GuiNpcStats extends GuiNPCInterface2 implements ITextfieldListener, IGuiData {
   ...
   public void init() {
      ...
      // melee properties
      MutableComponent hover = Component.translatable("stats.hover.melee");
      if (ais.aiDisabled) { hover.append(Component.translatable("hover.ai.disabled")); }
         ...
         hover.append(Component.translatable("guihint.npcmeleeprops.peaceful").withStyle(ChatFormatting.RED));
      }
      ...
      if (ais.aiDisabled) { hover.append(Component.translatable("hover.ai.disabled")); }
         ...
         hover.append(Component.translatable("guihint.npcrangedprops.peaceful").withStyle(ChatFormatting.RED));
      }
      ...
      addLabel(10, x0, (y += 34) + 5, "stats.fireimmune")
              .setSize(lw, 10);
      addYesNo(4, x1, y, npc.fireImmune())
              .setSize(bw, 20)
   ...
   @Override
   public void save() { Packets.sendServer(new SPacketMenuSave(EnumMenuType.STATS, stats.save(new CompoundTag()))); }

   @Override
   public void setGuiData(CompoundTag compound) {
      if (compound.contains("CreatureType", 3)) { stats.load(compound); }
      else if (compound.contains("MarkovGeneratorId", 3)) { display.load(compound); }
      else if (compound.contains("NpcInv", 9)) { inventory.load(compound); }
