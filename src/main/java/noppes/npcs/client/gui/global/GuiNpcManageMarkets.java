...
public class GuiNpcManageMarkets extends GuiNPCInterface2
        ...
        if (!dataDeals.isEmpty()) {
            ...
            for (Component key : dataDeals.keySet()) {
                ...
                totalInfo.add(Component.empty()
                        .append(Component.literal("ID: ").withStyle(ChatFormatting.GRAY))
                        .append(Component.literal("" + dealID).withStyle(ChatFormatting.RESET)));
                ...
                if (deal == null || !deal.isValid()) {
                    ...
                    if (tab == tabSelect) {
                        marcetInfo.add(Component.empty()
                                .append(Component.translatable("gui.sections").withStyle(ChatFormatting.GRAY))
                                .append(Component.literal(" ID: ").withStyle(ChatFormatting.GRAY))
                                .append(Component.literal("" + tab).withStyle(ChatFormatting.RESET))
                                .append(Component.literal("; ").withStyle(ChatFormatting.GRAY))
                                .append(Component.translatable("gui.name").withStyle(ChatFormatting.GRAY))
                                .append(Component.literal(": \"").withStyle(ChatFormatting.GRAY))
                                .append(Component.translatable(selectedMarcet.sections.get(tab).name).withStyle(ChatFormatting.RESET))
                                .append(Component.literal("\"").withStyle(ChatFormatting.GRAY)));
                    }
                    stack = dm.main;
                    if (deal.isCase()) {
                        ...
                        if (!deal.showInCase()) {
                            totalInfo.add(Component.translatable("market.case.show.false").withStyle(ChatFormatting.RED));
                            marcetInfo.add(Component.translatable("market.case.show.false").withStyle(ChatFormatting.RED));
                        }
                        deal.putHoverCaseItems(totalInfo, TooltipFlag.ADVANCED);
                        deal.putHoverCaseItems(marcetInfo, TooltipFlag.ADVANCED);
                    }
                    else {
                        totalInfo.add(Component.translatable("market.hover.product"));
                        marcetInfo.add(Component.translatable("market.hover.product"));
                        totalInfo.add(((MutableComponent) dm.main.getHoverName()).append(" x" + dm.count));
                        marcetInfo.add(((MutableComponent) dm.main.getHoverName()).append(" x" + dm.count + (deal.getMaxCount() > 0
                                ? Util.instance.getOldFormattedText(Component.translatable("market.hover.item.amount", "" + deal.getAmount()))
                                : "")));
                    }
                    // ItemStack
                    if (!dm.baseItems.isEmpty()) {
                        totalInfo.add(Component.translatable("market.hover.item." + (deal.showInCase() ? 1 : 0)));
                        for (ItemStack curr : dm.baseItems.keySet()) { totalInfo.add(((MutableComponent) dm.main.getHoverName()).append(" x" + dm.baseItems.get(curr))); }
                    }
                    ...
                    totalInfo.add(Component.translatable("market.deal.type." + dm.deal.getType()).withStyle(ChatFormatting.YELLOW));
                    totalInfo.add(Component.translatable("market.deal.chance").append(" " + dm.deal.getChance() + "%").withStyle(ChatFormatting.RESET));
                }
                if (tabSelect == tab) {
                    ...
                    if (deal != null && deal.isCase()) {
                        ...
                        if (objCase != null) {
                            try {
                                ...
                                if (res.isEmpty()) { objCase = null; }
                            }
                            catch (Exception e) { objCase = null; }
                        }
                        if (objCase == null) {
                            try {
                                Optional<Resource> res = minecraft.getResourceManager().getResource(Deal.defaultCaseOBJ);
                                if (res.isPresent()) { objCase = Deal.defaultCaseOBJ; }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        graphics.blit(GuiNPCTrader.ICONS, guiLeft + 252, guiTop + 189, u, v, 24, 24);
    }
