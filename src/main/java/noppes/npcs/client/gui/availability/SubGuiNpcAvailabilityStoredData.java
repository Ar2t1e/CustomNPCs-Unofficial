...
public class SubGuiNpcAvailabilityStoredData
	...
	public void initGui() {
		...
		for (AvailabilityStoredData sd : availability.storeddata) {
			Component type;
			switch(sd.type) {
				case ONLY: type = Component.literal("+").withStyle(TextFormatting.GREEN); break;
				case EXCEPT: type = Component.literal("-").withStyle(TextFormatting.RED); break;
				case SMALLER: type = Component.literal("<").withStyle(TextFormatting.YELLOW); break;
				case EQUAL: type = Component.literal("=").withStyle(TextFormatting.LIGHT_PURPLE); break;
				default: type = Component.literal(">").withStyle(TextFormatting.AQUA); break;
			}
			Component key = Component.empty().append(type)
					.append(Component.literal("\"").withStyle(TextFormatting.GOLD))
					.append(Component.literal(sd.key).withStyle(TextFormatting.RESET))
					.append(Component.literal("\"").withStyle(TextFormatting.GOLD));
			if (!sd.value.isEmpty()) {
				key.append(Component.literal(" - ").withStyle(TextFormatting.RESET))
						.append(Component.literal("\"").withStyle(TextFormatting.AQUA))
						.append(Component.literal(sd.value).withStyle(TextFormatting.RESET))
						.append(Component.literal("\"").withStyle(TextFormatting.AQUA));
			}
			data.put(key, sd);
