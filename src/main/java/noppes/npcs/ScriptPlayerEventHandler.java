...
public class ScriptPlayerEventHandler {
	...
	public void cnpcRightClickItem(PlayerInteractEvent.RightClickItem event) {
		...
		EntityPlayerMP player = (EntityPlayerMP) event.getEntityPlayer();
		CustomNpcs.debugData.start(player);
		if (player.isCreative() && player.isSneaking() && event.getItemStack().getItem() == CustomItems.scripted_item) {
	...
	public void cnpcBlockPlace(EntityPlaceEvent event) {
		...
		PlayerEvent.PlaceEvent ev = new PlayerEvent.PlaceEvent(handler.getPlayer(),
	...
	@SubscribeEvent
	public void cnpcTestingEvent(net.minecraftforge.event.entity.living.LivingEvent.LivingJumpEvent event) {
...
//dir = new java.io.File("E:/Sources/1.12.2/minecraft 1.12.2");
				...
				for (String key : found.keySet()) {
					...
					for (String fPath : map.keySet()) {
						...
						for (Integer line : map.get(fPath)) { LogWriter.info(javaC + " (class " + fPath.replace(".java", "") + " :" + line + "),"); }
					}
				}
				/**/
			}
