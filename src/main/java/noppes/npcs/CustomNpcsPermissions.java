...
public class CustomNpcsPermissions {
	...
	public static final Permission GLOBAL_PERMISSION = new Permission(CustomNpcs.MODID + ".global.permission", false);
						...
						for (Permission node : nodes) {
							if (node.name.equals(nodeName)) {
								...
								for (int i = 0; i < list.tagCount(); i++) {
									...
									if (node != ADMIN || (!name.equals("All") && !name.equals("Command Block"))) {
										permissions.get(node).add(list.getStringTagAt(i));
									}
								}
								hasInFile.add(node);
								found = true;
	...
	private static void save() {
		File file = new File(CustomNpcs.getWorldSaveDirectory(), "permissions.json");
		Map<String, List<String>> sortedPermissions = new LinkedHashMap<>();
		permissions.keySet().stream()
				.sorted((p1, p2) -> p1.name.compareToIgnoreCase(p2.name))
				.forEach(node -> sortedPermissions.put(node.name, permissions.get(node)));
		Gson gson = new GsonBuilder().setPrettyPrinting().disableHtmlEscaping().create();
		try (FileWriter writer = new FileWriter(file)) {
			gson.toJson(sortedPermissions, writer);
		}
		catch (Exception e) { LogWriter.error("Failed to save permissions via Gson", e); }
	}
	...
	public static void add(String nodeName, String playerName, EntityPlayerMP player) {
		if (!hasPermission(player, GLOBAL_PERMISSION)) {
	...
	public static void sendTo(EntityPlayerMP player) {
		...
		if (!hasPermission(player, GLOBAL_PERMISSION)) { Packets.send(player, new PacketGuiClose()); }
