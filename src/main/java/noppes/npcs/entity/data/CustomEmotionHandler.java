...
public class CustomEmotionHandler {
    ...
    public CustomEmotionHandler(LivingEntity main) {
        entity = main;
    }
    ...
    public void calculationEmotionData(float partialTicks) {
        ...
        int ticks = Math.max(0, (int) (entity.level().getGameTime() - startEmotionTime));
    ...
    public EmotionConfig tryRunEmotion(EmotionConfig emotion) {
        ...
        startEmotionTime = entity.level().getGameTime();
    ...
    public void load(CompoundTag compound) {
        baseEmotionId = compound.getInt("BaseEmotionId");
        if (!AnimationController.getInstance().hasEmotion(baseEmotionId)) {
            baseEmotionId = -1;
        }
    }

    public void save(CompoundTag compound) { compound.putInt("BaseEmotionId", baseEmotionId); }
    ...
    public void updateTime() {
        ...
        if (!AnimationController.getInstance().hasEmotion(activeEmotion.id) && stage != EnumAnimationStages.Ending && stage != EnumAnimationStages.Waiting) {
            ...
            startEmotionTime = entity.level().getGameTime() + 1;
            return;
        }
        int ticks = Math.max(0, (int) (entity.level().getGameTime() - startEmotionTime));
        int speed;
        if (activeEmotion.editFrame >= 0) {
            ...
            if (ticks >= activeEmotion.totalTicks) { startEmotionTime = entity.level().getGameTime() + 1; }
            return;
        }
        ...
        if (stage == EnumAnimationStages.Looping) {
            ...
            if (ticks >= speed) {
                ...
                if (frameId == 0) {
                    ...
                    startEmotionTime = entity.level().getGameTime() + activeEmotion.endingFrameTicks.get(frameId - 1) + 1;
                }
