...
public class CustomAnimationHandler {
    ...
    private void startEvent(AnimationEvent event) {
        ...
        else if (entity instanceof EntityPlayer) { handler = PlayerData.get((EntityPlayer) entity).scriptData; }
        ...
        if (!movementAnimation.isEmpty() && (activeAnimation == null || isMoving)) {
            ...
            if (base == null) {
                ...
                if (entity.getRidingEntity() != null ||
                        (entity instanceof EntityNPCInterface && ((EntityNPCInterface) entity).currentAnimation == AnimationType.SIT.get())) {
                    base = AnimationKind.SITTING;
                }
                // SLEEPING
                if (entity.isEntityAlive() && entity.isPlayerSleeping()) { base = AnimationKind.SLEEPING; }
                // NORMAL + sneak / sitting
                if (base == null && entity.onGround) {
                    boolean isSneaking = entity.isSneaking() ||
                            (entity instanceof EntityNPCInterface && ((EntityNPCInterface) entity).ais.animationType == AnimationType.SNEAK.get());
                    if (isMoving) { base = isSneaking ? AnimationKind.SNEAK_WALK : AnimationKind.WALKING; }
                    else { base = isSneaking ? AnimationKind.SNEAK_STAND : AnimationKind.STANDING; }
